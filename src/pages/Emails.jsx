...
export default function Emails({ onNavigate }) {
  const [logs, setLogs] = useState([]);
  const [customers, setCustomers] = useState([]);
  const [loading, setLoading] = useState(true);
  const [sending, setSending] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [form, setForm] = useState({ customerId: '', subject: '', body: '', type: 'welcome' });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [logData, customerData] = await Promise.all([
        getEmailLogs({ limit: 1000 }),
        getCustomers({ limit: 1000 }),
      ]);
      setLogs(logData.logs);
      setCustomers(customerData.customers);
    } catch (error) {
      addToast(error.response?.data?.message || 'Failed to load emails', 'error');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Emails"
        subtitle="Send and track email communications"
        action={
          <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16 text-dark-400 dark:text-dark-500">
          <Loader2 className="animate-spin mr-2" size={20} /> Loading emails...
        </div>
      ) : (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-dark-50 dark:bg-dark-700">
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-dark-500 uppercase dark:text-dark-400">Subject</th>
          ...
          <tbody className="divide-y divide-dark-200 dark:divide-dark-700">
            {logs.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-dark-400 dark:text-dark-500">No emails sent yet</td></tr>
            ) : (
              logs.map(log => (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Send Email" size="md">
        <div className="space-y-4">
          <div>
            ...
            >
              ...
              {customers.map(c => <option key={c._id} value={c._id}>{c.name} ({c.email || 'no email on file'})</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-dark-700 mb-1 dark:text-gray-300">Email Type</label>
            <select
              value={form.type}
          ...
          <div>
            <label className="block text-sm font-medium text-dark-700 mb-1 dark:text-gray-300">Subject <span className="text-dark-400 font-normal">(leave blank to use the {form.type} template)</span></label>
            <input
              value={form.subject}
          ...
          <div>
            <label className="block text-sm font-medium text-dark-700 mb-1 dark:text-gray-300">Message <span className="text-dark-400 font-normal">(leave blank to use the {form.type} template)</span></label>
            <textarea
              value={form.body}
